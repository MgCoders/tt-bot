...
class Connection(object):
    ...
    def getProjectTimeTrackingWorkTypes(self, projectId):
        try:
            cont = self._get('/admin/project/' + projectId + '/timetracking/worktype')
            return youtrack.ProjectTimeTrackingWorkTypes(cont, self)
        except youtrack.YouTrackException, e:
            if e.response.status != 404:
                raise e
